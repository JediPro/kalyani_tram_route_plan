...
mtx_popn_sample <- tibble(indiv_index = 1:10) %>%
  # Create duplicate rows for each line within each individual
  mutate(line = list(c(1:param_n_line))) %>% 
  unnest_longer(col = line) %>% 
  # Sample randomly selected number of points for each line from station indices
  # First set number of points to fetch for each line
  mutate(num_station = map_int(.x = line, 
                               .f = function(x) {
                                 sample(x = 2:max(sf_soln_stations$id), 
                                        size = 1, replace = FALSE)},
                               .progress = TRUE),
         # Next, fetch list of station indices
         station_id = map(.x = num_station, 
                          .f = function(x) {
                            # Define vector of stations
                            vec_station <- sample(x = sf_soln_stations$id, 
                                                  size = x, replace = FALSE)
                            # Create vector of 0s with length equal to length of pad
                            vec_zero <- rep(0, max(sf_soln_stations$id) - x)
                            # Return final padded vector
                            return(c(vec_station, vec_zero))
                            }, 
                          .progress = TRUE)) %>% 
  # Unnest column
  unnest_longer(col = station_id) %>% 
  # Remove unwanted column
  select(-c(line, num_station)) %>% 
  # Assign column names
  group_by(indiv_index) %>%
  mutate(col_name = row_number()) %>%
  ungroup() %>%
