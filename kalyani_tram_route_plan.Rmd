...
data_lrt_pop <- table_lrt_systems %>% 
  ...
  left_join(y = table_metro_systems %>% group_by(city, country) %>% 
              summarise(length = sum(length), .groups = "drop"),
            by = c("city", "country"), suffix = c("", "_metro")) %>% 
...
# Prediction
10^(mod_length_lines$coefficients[1] + (mod_length_lines$coefficients[2] * log10(30)))
...
# Prediction
10^(mod_lines_stations$coefficients[1] + (mod_lines_stations$coefficients[2] * log10(10)))

# Population vs Length -------------------------------
# Plot
plot_popn_length <- data_lrt_pop %>% 
  filter(is.na(length_metro) & popn >= 2e+4) %>% 
  # plot
  ggplot() +
  # points
...
mod_popn_length <- lm(formula = log10(length) ~ log10(popn), 
                          data = data_lrt_pop %>%
                        filter(is.na(length_metro) & popn >= 2e+4), na.action = na.omit)
# R-squared
