...
fx_ga_fitness <- function(vec_individual){
  ...
  data_station_popn_served <-  sf_indiv_stations %>% 
    ...
    # Calculate multiplier for demand based on distance
    (function(a) exp(-(a^2)/(param_sd_sat_dist^2))) %>%
    # Normalize so that each column sums up to 1, ensures popn is fully divided
    sweep(2, colSums(.), `/`) %>% 
    ...
    # Convert to tibble
    as_tibble() %>% 
    rename(popn_served = V1) %>% 
    # Round figure
    mutate(popn_served = round(popn_served)) %>% 
  ...
  sc_total_popn_served <- data_station_popn_served %>% 
    ...
    as_tibble() %>% 
    select(total_popn) %>% 
    sum()/sc_total_popn_served
    # Scale to total popn
    `%/%` (sc_total_demand)
  
  # print("Fitness calculation complete!")
  
  return(sc_total_popn_served)
}
