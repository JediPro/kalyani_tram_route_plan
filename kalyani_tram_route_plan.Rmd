...
# Prediction
10^(mod_length_stations$coefficients[1] + (mod_length_stations$coefficients[2] * log10(1)))
...
# Prediction
10^(mod_length_lines$coefficients[1] + (mod_length_lines$coefficients[2] * log10(7)))
...
# Prediction
10^(mod_lines_stations$coefficients[1] + (mod_lines_stations$coefficients[2] * log10(1)))
...
# Prediction
10^(mod_popn_length$coefficients[1] + (mod_popn_length$coefficients[2] * log10(1e+5)))
...
# Fetch road network for city in question ---------------------
# Create BB for Kalyani
ymax <- 22.994
ymin <- 22.957
xmax <- 88.471
xmin <- 88.421
...
sf_demand_point <- tribble(
  ...
  "Kalyani Railway Station", 25000, 22.96929, 88.46738,
  "Kalyani Silpanchal Railway Station", 500, 22.97625, 88.45643,
  "Kalyani Ghoshpara Railway Station", 1000, 22.98163, 88.43940,
  "Kalyani Simanta Railway Station", 200, 22.98747, 88.42793,
  "JNM Hospital", 10000, 22.97359, 88.45592,
  "Vidyasagar Manch", 500, 22.97090, 88.46263,
  "Kalyani Lake Park", 500, 22.97273, 88.44338,
  "Kalyani ITI More", 2000, 22.98010, 88.43795,
  "Kalyani Central Park", 3000, 22.97554, 88.43496,
  "Buddha Park More", 2000, 22.96325, 88.42791,
  "Kalyani Stadium", 5000, 22.97496, 88.44936,
  "Kalyani Municipality", 1000, 22.97693, 88.44724,
  "A-2 Market", 3000, 22.97335, 88.46369,
  "A-11 Market", 2000, 22.96443, 88.45425,
  "B-8 Market", 3000, 22.98277, 88.4328,
  "B-11 Market", 3000, 22.97599, 88.44339,
  "Kalyani University", 2000, 22.98567, 88.4455,
  "JIS College", 2500, 22.95954, 88.44801,
  "Springdale School", 1000, 22.97733, 88.45174,
  "Kalyani Experimental School", 1500, 22.97814, 88.44438,
  ...
  # Convert to sf
  st_as_sf(coords = c("lon", "lat"), crs = st_crs(bbox_polygon))

ggplot() +
  tidyterra::geom_spatraster(data = rs_built, aes(fill = res)) +
  geom_sf(data = sf_city_road) +
  geom_sf(data = sf_demand_point, size = 3, colour = "red") +
  scale_fill_viridis_c()

ggplot() +
  geom_sf(data = sf_city_road %>% filter(!highway %in% c("unclassified", "residential")))
```
