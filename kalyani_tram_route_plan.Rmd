...
fx_ga_fitness <- function(vec_individual){
  ...
  data_station_point_served <- sf_indiv_stations %>% 
    ...
    # sweep(MARGIN = 2, STATS = colSums(.), FUN = `/`) %>% 
    # Multiply each with the corresponding demand
    sweep(MARGIN = 2, sf_demand_point$demand, `*`) %>%
    # Round out the values to floor integer to ensure only whole individuals are counted
  ...
  data_station_popn_served <-  sf_indiv_stations %>% 
    ...
    # Calculate multiplier for demand based on distance
    (function(a) exp(-(a^2)/(param_sd_sat_dist^2))) %>%
    # Multiply each with the corresponding population
