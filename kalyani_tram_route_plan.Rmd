...
fx_generate_line_layout <- function(){
  ...
  table_stations <- tibble(id = 1:max(sf_soln_stations$id)) %>% 
    mutate(line_id = case_when(id %in% sample(x = 2:max(sf_soln_stations$id), 
    ...
    mutate(num_station = sample(x = 2:max(sf_soln_stations$id), 
           ...
           station_id = map2(.x = id, .y = num_station, 
                             .f = function(x,y){
                               ...
                               remain_vec <- setdiff(1:max(sf_soln_stations$id), curr_vec)
                               ...
                               vec_zero <- rep(0, max(sf_soln_stations$id) - length(append_vec))
